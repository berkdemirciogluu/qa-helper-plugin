...
describe('snapshot content script', () => {
  it('listener is registered for TAKE_SNAPSHOT message', () => {
    expect(capturedListener).not.toBeNull();
  });

  it('TAKE_SNAPSHOT: DOM is serialized', async () => {
  ...
  it('TAKE_SNAPSHOT: localStorage is dumped', async () => {
    const result = await dispatch('TAKE_SNAPSHOT');
    expect(result.success).toBe(true);
  ...
  it('TAKE_SNAPSHOT: sessionStorage is dumped', async () => {
    const result = await dispatch('TAKE_SNAPSHOT');
    expect(result.success).toBe(true);
  ...
  it('does not return true for unknown action (listener only handles TAKE_SNAPSHOT)', () => {
    ...
    // Should not return true for non-TAKE_SNAPSHOT messages
